...
class NationalRailScheduleCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, token: str, station: str, destinations: list[str]):
        ...
        self._external_high_frequency_request = False  # Flag for MonitoredTrainSensor to request fast polling
        self._high_freq_timer_cancel = None  # Timer for active 1-minute polling
        _LOGGER.debug("Coordinator initialized for %s. Sensor Name: %s", self.station_name, self.sensor_name)

    def request_high_frequency_polling(self, enable: bool = True):
        """Request high-frequency polling to be enabled or disabled.

        Called by MonitoredTrainSensor when a tracked train is approaching departure.

        Args:
            enable: True to enable high-frequency polling, False to disable
        """
        if enable != self._external_high_frequency_request:
            self._external_high_frequency_request = enable
            if enable:
                _LOGGER.debug("COORD [%s]: High-frequency polling requested by MonitoredTrainSensor", self.name)
                # Start a repeating timer to force refreshes every 1 minute
                if self._high_freq_timer_cancel is None:
                    async def _high_freq_refresh(now):
                        """Callback for high-frequency timer"""
                        _LOGGER.debug("COORD [%s]: High-frequency timer triggered, forcing refresh", self.name)
                        # Use async_refresh() to force immediate update bypassing time checks
                        await self.async_refresh()

                    self._high_freq_timer_cancel = async_track_time_interval(
                        self.hass,
                        _high_freq_refresh,
                        timedelta(minutes=HIGH_FREQUENCY_INTERVAL)
                    )
                    _LOGGER.info("COORD [%s]: Started high-frequency timer (every %d min)", self.name, HIGH_FREQUENCY_INTERVAL)
            else:
                _LOGGER.debug("COORD [%s]: High-frequency polling request cleared", self.name)
                # Cancel the high-frequency timer
                if self._high_freq_timer_cancel is not None:
                    self._high_freq_timer_cancel()
                    self._high_freq_timer_cancel = None
                    _LOGGER.info("COORD [%s]: Stopped high-frequency timer", self.name)


    def _process_api_response(self, fetched_data: dict | None) -> dict:
        """
        Processes the raw data fetched from the API (or None if fetch failed at API level)
        and transforms it into the structured format used by the coordinator and sensors.
        """
        _LOGGER.debug("COORD [%s]: Starting _process_api_response. fetched_data is None: %s",
                      self.name, fetched_data is None)

        # --- Handle Case Where API Fetch Itself Returned None (before client-side errors) ---
        if fetched_data is None:
            _LOGGER.warning("COORD [%s]: _process_api_response received None for fetched_data. Indicating API fetch problem.", self.name)
            # This structure is for when the API call itself (e.g. HTTP error, timeout before client processing) failed to yield data.
            # The client.py's async_get_data usually raises an exception or returns a dict with an 'error' key
            # for such cases, so fetched_data being None here might be less common if client handles all errors.
            ...
            return {
                ...
                "api_error": "API Fetch Returned No Data", # More specific error
                "next_train_scheduled": "API Error",
                "next_train_expected": "API Error",
                "perturbations": False,
                "nrcc_messages": None,
                # Added to signal the caller that processing itself had an issue at the very start
                "processing_error_details": "Fetched data was None before processing."
            }

        # --- Initialize `processed_data` structure ---
        # This will be populated or updated based on fetched_data
        processed_data = {
            "name": self.sensor_name,
            "description": self.description,
            "friendly_name": self.friendly_name,
            "station": fetched_data.get("station_name", self.station_name), # Use actual from data if available
            "station_code": self.station,
            "trains": [], # Default to empty list
            "last_update": dt_util.now().isoformat(),
            ...
            "perturbations": False,
            "api_error": fetched_data.get("error"), # Error reported by client.py
            "nrcc_messages": None,
        }

        try:
            # --- Extract NRCC Messages ---
            # (Assuming nrccMessages structure from your client.py's process_data output)
            # client.py process_data returns: status["nrccMessages"] = messages_list if messages_list else None
            # So fetched_data.get("nrccMessages") should be a list of strings or None.
            nrcc_messages_from_client = fetched_data.get("nrccMessages")
            if isinstance(nrcc_messages_from_client, list) and nrcc_messages_from_client:
                processed_data["nrcc_messages"] = nrcc_messages_from_client
            else:
                processed_data["nrcc_messages"] = None # Ensure it's None if empty or not a list

            _LOGGER.debug("COORD [%s]: NRCC Messages from client: %s", self.name, nrcc_messages_from_client)


            # --- Handle API Error (reported by client) or No Trains from client ---
            if processed_data["api_error"]:
                _LOGGER.error("COORD [%s]: API error reported by client: %s", self.name, processed_data["api_error"])
                processed_data["next_train_scheduled"] = "API Error"
                processed_data["next_train_expected"] = "API Error"
                processed_data["trains"] = [] # Ensure trains list is empty
            elif not isinstance(fetched_data.get("trains"), list) or not fetched_data.get("trains"):
                # This checks if 'trains' is not a list or is an empty list
                trains_data = fetched_data.get("trains")
                log_msg = "No trains found in data"
                if not isinstance(trains_data, list) :
                    log_msg = f"Trains data is not a list (type: {type(trains_data).__name__})"
                _LOGGER.info("COORD [%s]: %s for %s.", self.name, log_msg, self.station_name)
                processed_data["next_train_scheduled"] = "No Trains Found"
                processed_data["next_train_expected"] = "No Trains Found"
                processed_data["trains"] = [] # Ensure trains list is empty
            else:
                # We have a list of trains, proceed to process them
                processed_data["trains"] = fetched_data.get("trains", []) # Assign full list for attributes
                first_valid_train = None
                earliest_departure_dt = None

                for train in processed_data["trains"]:
                    # --- Perturbation Check for any train ---
                    processed_data["perturbations"] = processed_data["perturbations"] or train.get("perturbation", False)

                    # --- Check if this train is cancelled ---
                    # client.py sets "expected" to "Cancelled" string or is_cancelled_bool
                    # We rely on the "expected" string for this check primarily here.
                    ...
                    if isinstance(expected_str_check, str) and expected_str_check.lower() == "cancelled":
                        is_train_cancelled = True
                    # Also consider the boolean flag if available
                    if train.get("isCancelled") is True: # Explicit check for True
                        is_train_cancelled = True

                    if is_train_cancelled:
                        _LOGGER.debug("COORD [%s]: Skipping cancelled train: %s", self.name, train.get("serviceId", train.get("scheduled")))
                        continue # Skip this cancelled train for "next train" selection

                    # --- Parse Times ---
                    scheduled_str = train.get("scheduled")
                    expected_str = train.get("expected") # Original expected string ("On time", "10:05", "Delayed")

                    scheduled_dt = self._parse_time_safely(scheduled_str)
                    expected_dt = self._parse_time_safely(expected_str) # Returns datetime or None if 'expected_str' is a status

                    # --- Determine Effective Departure Time for this train ---
                    effective_departure_dt = None
                    if expected_dt: # If 'expected' was a parsable time
                        effective_departure_dt = expected_dt
                    elif isinstance(expected_str, str) and expected_str.lower() == "on time" and scheduled_dt:
                        effective_departure_dt = scheduled_dt
                    elif scheduled_dt: # Fallback to scheduled if expected is not a time or "On time"
                        effective_departure_dt = scheduled_dt
                    else:
                        _LOGGER.debug("COORD [%s]: Skipping train due to unparsable times: Sched='%s', Exp='%s'",
                                      self.name, scheduled_str, expected_str)
                        continue # Skip train if no effective time can be determined

                    # --- Check if this is the earliest valid (non-cancelled, parsable) train ---
                    if first_valid_train is None or effective_departure_dt < earliest_departure_dt:
                        ...
                        _LOGGER.debug("COORD [%s]: New candidate for first_valid_train: %s, Effective: %s",
                                      self.name, train.get("serviceId", scheduled_str), effective_departure_dt)


                # --- Populate data based on the `first_valid_train` found ---
                if first_valid_train:
                    _LOGGER.debug("COORD [%s]: Selected first valid train: %s (Scheduled: %s)",
                                  self.name, first_valid_train.get("serviceId", "N/A"), first_valid_train.get("scheduled"))
                    
                    # Get scheduled and expected times from the chosen train
                    final_scheduled_time_output = first_valid_train.get("scheduled")
                    final_expected_time_output = first_valid_train.get("expected") # This could be "On time", "Delayed", or HH:MM

                    processed_data["next_train_scheduled"] = final_scheduled_time_output
                    processed_data["next_train_expected"] = final_expected_time_output
                    ...
                    # Destination: The first destination in its 'destinations' list
                    ...
                    # No valid (non-cancelled, parsable time) trains found after checking all
                    _LOGGER.warning("COORD [%s]: No valid (non-cancelled, parsable time) trains found for %s.",
            ...
            _LOGGER.debug("COORD [%s]: Finished processing API response. Next Scheduled: %s, Next Expected: %s, Perturbations: %s, NRCC: %s",
                          self.name, processed_data.get("next_train_scheduled"),
                          processed_data.get("next_train_expected"), processed_data.get("perturbations"),
                          bool(processed_data.get("nrcc_messages")))

            return processed_data

        except Exception as processing_err:
            _LOGGER.exception("COORD [%s]: CRITICAL - Error during data processing phase in _process_api_response: %s",
                              self.name, processing_err)
            # Return a clear error structure if processing fails
            # This helps the calling methods (_async_update_data/_force) to decide
            # whether to use old data or raise UpdateFailed.
            return {
                "name": self.sensor_name,
                "description": self.description,
                "friendly_name": self.friendly_name,
                "station": self.station_name, # Use known station name
                "station_code": self.station,
                "trains": [], # No trains if processing failed
                "last_update": dt_util.now().isoformat(),
                "api_error": f"Data Processing Failed: {processing_err}", # Indicate processing error
                "next_train_scheduled": "Processing Error",
                "next_train_expected": "Processing Error",
                "perturbations": False,
                "nrcc_messages": None,
                "processing_error_details": str(processing_err) # Specific detail for UpdateFailed
            }
    ...
    async def _async_update_data(self):
        ...
        now_timestamp = time_module.time() # Use a consistent timestamp for 'now'
        now_dt = dt_util.now()

        should_fetch = False
        reason = ""

        # --- Adjust update interval based on high-frequency request ---
        # This must happen BEFORE the fetch decision logic to ensure interval is always correct
        interval_changed = False
        if self._external_high_frequency_request:
            # Dynamically adjust update interval to poll more frequently
            new_interval = timedelta(minutes=HIGH_FREQUENCY_INTERVAL)
            if self.update_interval != new_interval:
                _LOGGER.info("Switching to high-frequency polling (%d min interval) for %s",
                             HIGH_FREQUENCY_INTERVAL, self.station_name)
                self.update_interval = new_interval
                interval_changed = True
        else:
            # Not in high-frequency mode, use normal interval
            normal_interval = timedelta(minutes=REFRESH)
            if self.update_interval != normal_interval:
                _LOGGER.info("Switching back to normal polling (%d min interval) for %s",
                             REFRESH, self.station_name)
                self.update_interval = normal_interval
                interval_changed = True

        # CRITICAL FIX: If interval changed, we need to trigger a reschedule
        # DataUpdateCoordinator doesn't automatically reschedule when update_interval changes
        # We force a fetch to happen by setting should_fetch = True
        if interval_changed:
            should_fetch = True
            reason = f"Update interval changed to {self.update_interval.total_seconds() / 60:.0f} min - forcing immediate fetch to reschedule"
            _LOGGER.debug("COORD [%s]: %s", self.name, reason)

        # --- Determine if fetch is needed ---
        if self.last_data_refresh is None:
            should_fetch = True
            reason = "Initial data fetch"
        elif (now_timestamp - self.last_data_refresh) >= POLLING_INTERVALE * 60:
            should_fetch = True
            reason = f"Normal polling interval ({POLLING_INTERVALE} min) elapsed"
        elif self.data and self._external_high_frequency_request:
            # High-frequency polling requested - check if it's time based on HIGH_FREQUENCY_INTERVAL
            if (now_timestamp - self.last_data_refresh) >= HIGH_FREQUENCY_INTERVAL * 60:
                should_fetch = True
                reason = "High-frequency polling requested by sensor (tracked train imminent)"

        # --- Fetch Data (if needed) ---
        fetched_data_from_api = None # Will hold data directly from my_api.async_get_data()
        if should_fetch:
            _LOGGER.info("Fetching new data for %s. Reason: %s", self.station_name, reason)
            try:
                async with async_timeout.timeout(30): # Timeout for the API call
                    fetched_data_from_api = await self.my_api.async_get_data()
                # Log details about the fetched data
                _LOGGER.debug("COORD [%s]: Raw fetched_data_from_api received (is None: %s). Keys: %s",
                              self.name, fetched_data_from_api is None,
                              fetched_data_from_api.keys() if fetched_data_from_api else "N/A")
                self.last_data_refresh = now_timestamp # Update refresh time on successful fetch attempt
                _LOGGER.debug("Successfully fetched new data for %s (%s trains in raw response)",
                              self.station_name,
                              len(fetched_data_from_api.get("trains", [])) if fetched_data_from_api else 0)

            except Exception as err: # Covers httpx errors, SOAPFaults, NationalRailClientExceptions from client
                _LOGGER.error("Error during API data fetch for %s: %s", self.station_name, err)
                if self.data is None: # No previous data to serve
                    raise UpdateFailed(f"Initial data fetch failed for {self.station_name}: {err}") from err
                else: # Has previous data, log warning and return it with an error marker
                    _LOGGER.warning("Fetch failed for %s, returning previous data.", self.station_name)
                    # Add/update api_error on existing self.data to inform sensors
                    self.data["api_error"] = f"Fetch Failed: {type(err).__name__} - {err}"
                    self.data["last_update"] = dt_util.now().isoformat() # Mark this attempt
                    return self.data # Return existing data

            # --- Pass to common processor ---
            # This happens if the fetch attempt itself (HTTP, API call) didn't raise an exception.
            # `fetched_data_from_api` could still be None if `my_api.async_get_data()` returned None,
            # or it could contain an 'error' key from client-side validation.
            processed_result = self._process_api_response(fetched_data_from_api)

            # Check if the processing itself encountered an error (signaled by "processing_error_details")
            # or if client reported an error that was passed through _process_api_response
            if "processing_error_details" in processed_result or processed_result.get("api_error"):
                error_detail = processed_result.get("processing_error_details", processed_result.get("api_error", "Unknown processing issue"))
                _LOGGER.error("COORD [%s]: Data processing failed or API error reported after fetch. Error: %s",
                              self.name, error_detail)
                if self.data: # If previous data exists, use it, but mark the error
                    self.data["processing_error"] = processed_result.get("api_error", "Processing Failed") # Add/update marker
                    self.data["last_update"] = dt_util.now().isoformat()
                    _LOGGER.warning("COORD [%s]: Returning previous data due to processing/API error.", self.name)
                    return self.data
                # No previous data and processing failed or API error from client
                raise UpdateFailed(f"Data processing or API error for {self.station_name}: {error_detail}")

            # Successfully processed new data
            self.data = processed_result
            return self.data

        else: # No fetch needed based on timing/conditions
            if self.data is None: # Should not happen if initial fetch logic is correct, but safety check
                _LOGGER.warning("COORD [%s]: No cached data available and no fetch condition met. Forcing fetch.", self.name)
    ...
    async def _async_update_data_force(self):
        ...
        _LOGGER.info("COORD [%s]: Forcing data fetch for %s", self.name, self.station_name)
        fetched_data_from_api = None
        now_timestamp = time_module.time()

        try:
            async with async_timeout.timeout(30): # Timeout for the API call
                fetched_data_from_api = await self.my_api.async_get_data()
            _LOGGER.debug("COORD [%s]: Raw fetched_data_from_api (forced) (is None: %s). Keys: %s",
                          self.name, fetched_data_from_api is None,
                          fetched_data_from_api.keys() if fetched_data_from_api else "N/A")
            self.last_data_refresh = now_timestamp # Update refresh time
            _LOGGER.debug("Successfully fetched data during forced update for %s (%s trains in raw response)",
                          self.station_name,
                          len(fetched_data_from_api.get("trains", [])) if fetched_data_from_api else 0)
        except Exception as err:
            _LOGGER.error("Error during forced API data fetch for %s: %s", self.name, err)
            if self.data is None: # No previous data to serve
                raise UpdateFailed(f"Forced fetch failed for {self.station_name} and no previous data exists: {err}") from err
            else: # Has previous data, log warning and return it with an error marker
                _LOGGER.warning("Forced fetch failed for %s, returning previous data.", self.name)
                self.data["api_error"] = f"Forced Fetch Failed: {type(err).__name__} - {err}"
                self.data["last_update"] = dt_util.now().isoformat()
                return self.data
        ...
        # Check if the processing itself encountered an error or if client reported an error
        if "processing_error_details" in processed_result or processed_result.get("api_error"):
            error_detail = processed_result.get("processing_error_details", processed_result.get("api_error", "Unknown processing issue"))
            _LOGGER.error("COORD [%s]: Data processing or API error reported after forced fetch. Error: %s",
                          self.name, error_detail)
            if self.data: # If previous data exists, use it, but mark the error
                self.data["processing_error"] = processed_result.get("api_error", "Processing Failed") # Add/update marker
                self.data["last_update"] = dt_util.now().isoformat()
                _LOGGER.warning("COORD [%s]: Returning previous data due to processing/API error after forced fetch.", self.name)
                return self.data
            # No previous data and processing failed or API error from client
            raise UpdateFailed(f"Data processing or API error for {self.station_name} after force fetch: {error_detail}")

        # Successfully processed new data from forced fetch
        _LOGGER.debug("COORD [%s]: Assigning processed_data from forced fetch to self.data. Keys: %s",
                      self.name, processed_result.keys() if processed_result else "None")
        self.data = processed_result
        return self.data
...
class MonitoredTrainSensor(SensorEntity):
    ...
    def should_poll(self) -> bool:
        """Enable polling when in en-route tracking mode, otherwise rely on source sensor."""
        return self._enroute_tracking_enabled

    @property
    def coordinator(self) -> NationalRailScheduleCoordinator:
        """Return the coordinator (property for backward compatibility)."""
    ...
    def __init__(
        ...
        self.hass = hass
        self._coordinator = coordinator  # Store with underscore for consistency
        ...
        self._update_in_progress: bool = False  # Prevent re-entrant updates

        # En-route tracking (when train has departed and vanished from departure board)
        self._enroute_tracking_enabled: bool = False
        self._last_enroute_check: datetime | None = None
        self._enroute_check_interval = timedelta(minutes=5)  # Check every 5 minutes
        self._enroute_failure_count: int = 0
        self._max_enroute_failures: int = 3
        self._has_real_service_id: bool = False  # Track if we have a real API serviceId vs constructed one
    ...
    async def _async_fetch_enroute_data(self) -> dict | None:
        """Fetch en-route data for the tracked train using GetServiceDetails endpoint.

        Returns:
            dict with en-route data if successful, None if failed
        """
        entity_id_log = self.entity_id or self._attr_name

        if not self._tracked_train_id:
            _LOGGER.warning("%s: Cannot fetch en-route data - no tracked train ID", entity_id_log)
            return None

        try:
            _LOGGER.debug("%s: Fetching en-route data for service %s", entity_id_log, self._tracked_train_id)

            # Call the track_train_enroute service
            ...
            if not response.get("success"):
                ...
                _LOGGER.warning("%s: En-route tracking failed: %s", entity_id_log, error_msg)
                return None

            _LOGGER.debug("%s: En-route data received: %s", entity_id_log, response)
            return response

        except Exception as err:
            _LOGGER.error("%s: Error fetching en-route data: %s", entity_id_log, err, exc_info=True)
            return None
    ...
    async def _async_fetch_arrival_data(self) -> dict | None:
        """Fetch arrival board data for the tracked train using GetArrBoardWithDetails.

        This is used when the train has departed and vanished from the departure board.
        We poll the arrival board at the destination, filtered by origin station.

        Returns:
            dict with arrival data if successful, None if failed
        """
        entity_id_log = self.entity_id or self._attr_name

        # Check we have the required station codes
        if not self._destination_crs:
            _LOGGER.warning("%s: Cannot fetch arrival data - no destination CRS code", entity_id_log)
            return None

        if not self._origin_crs:
            _LOGGER.warning("%s: Cannot fetch arrival data - no origin CRS code", entity_id_log)
            return None

        try:
            _LOGGER.debug(
                "%s: Fetching arrival board at %s from %s",
                entity_id_log,
                self._destination_crs,
                self._origin_crs
            )
            ...
            # Get API token from coordinator's client
            api_token = self._coordinator.my_api.api_token

            # Create temporary client for the destination station
            temp_client = NationalRailClient(
                api_token=api_token,
                station=self._destination_crs,
                destinations=None  # No filtering needed
            )

            # Fetch arrival board filtered by origin
            raw_arrivals = await temp_client.get_raw_arrivals(
                destination_crs=self._destination_crs,
                origin_crs=self._origin_crs
            )

            if not raw_arrivals:
                _LOGGER.debug("%s: No arrival board data received", entity_id_log)
                return None

            # Process the arrival data
            arrival_data = temp_client.process_arrival_data(
                raw_arrivals,
                target_arrival_time=self._tracked_arrival_dt
            )

            if not arrival_data or not arrival_data.get("arrivals"):
                _LOGGER.debug("%s: No arrivals found on board", entity_id_log)
                return None

            # Find our train in the arrivals
            # Look for train with arrival time close to our expected arrival
            best_match = None
            if self._tracked_arrival_dt:
                min_time_diff = timedelta(hours=1)  # Max 1 hour difference
                for arrival in arrival_data["arrivals"]:
                    sched_arrival = arrival.get("scheduled_arrival")
                    if isinstance(sched_arrival, datetime):
                        time_diff = abs(sched_arrival - self._tracked_arrival_dt)
            ...
            if best_match:
                _LOGGER.info(
                    "%s: Found train on arrival board - Platform: %s, ETA: %s",
                    entity_id_log,
                    best_match.get("platform", "TBC"),
                    best_match.get("expected_arrival", "Unknown")
                )
                return {
                    "success": True,
                    "arrival": best_match,
                    "station": arrival_data["station"]
                }
            else:
                _LOGGER.debug("%s: Train not found on arrival board (checked %d arrivals)",
                             entity_id_log, len(arrival_data["arrivals"]))
                return None

        except Exception as err:
            _LOGGER.error("%s: Error fetching arrival data: %s", entity_id_log, err, exc_info=True)
            return None
    ...
    async def async_added_to_hass(self) -> None:
        ...
        def source_state_listener(event: Event) -> None:
            ...
            elif self._initialized and not self._tracking_stopped and source_has_trains and not source_is_error:
                # CRITICAL: When actively tracking a train, always update even if trains data is identical
                # This allows us to detect when the tracked train is no longer in the feed (has departed)
                trigger_update = True
                trigger_reason = f"Periodic Check (Tracking Active, {new_state.state})"
                _LOGGER.debug("%s: Actively tracking - scheduling update to check if train still in feed.", entity_id_log)
        ...
        # Add coordinator update listener to catch all data updates
        @callback
        def coordinator_update_listener() -> None:
            """Handle coordinator data updates even if source sensor state doesn't change."""
            # Skip if already updating to prevent infinite loop
            if self._update_in_progress:
                return

            if self._initialized and not self._tracking_stopped:
                entity_id_log = self.entity_id or self._attr_name or f"monitored_train_{self._attr_unique_id}"
                _LOGGER.debug("%s COORDINATOR LISTENER: Coordinator updated, scheduling check", entity_id_log)
                self._attr_extra_state_attributes["last_update_trigger"] = "Coordinator Update"
                self.hass.async_create_task(self.async_update())
        ...
        try:
            ...
            listeners.append(self._coordinator.async_add_listener(coordinator_update_listener))
            _LOGGER.debug("%s: Added coordinator update listener", entity_id_log)
    ...
    async def async_update(self) -> None:
        ...
        # Prevent re-entrant updates (infinite loop protection)
        if self._update_in_progress:
            return
        ...
        try:
            ...
            if self._tracking_stopped and not self._reset_timer_cancel:
                ...
                # Permanent/latching monitored sensors must recover for future services.
                # Temporary ad-hoc sensors (sensor.temp_*) should stay terminal until they expire.
                is_temporary_sensor = (
                    (self.entity_id or "").startswith("sensor.temp_")
                    or (self._attr_name or "").startswith("temp_")
                )
                if not is_temporary_sensor:
                    _LOGGER.info(
                        "%s: Auto-scheduling reset for persistent monitored sensor after terminal state.",
                        entity_id_log,
                    )
            ...
            # --- En-Route Tracking Mode ---
            if self._enroute_tracking_enabled:
                _LOGGER.debug("%s: En-route tracking enabled. Checking if update needed.", entity_id_log)

                # Check if we should fetch en-route data
                should_fetch_enroute = False
                if self._last_enroute_check is None:
                    should_fetch_enroute = True
                    _LOGGER.debug("%s: First en-route check", entity_id_log)
                elif (update_start_time - self._last_enroute_check) >= self._enroute_check_interval:
                    should_fetch_enroute = True
                    _LOGGER.debug("%s: En-route check interval elapsed", entity_id_log)

                if should_fetch_enroute:
                    tracked_details = self._attr_extra_state_attributes.get("tracked_train_details", {})
                    if not isinstance(tracked_details, dict):
                        tracked_details = {}
                    rtt_uid = tracked_details.get("rtt_uid")

                    rtt_position = None
                    if rtt_uid:
                        _LOGGER.info("%s: Polling RTT for UID %s", entity_id_log, rtt_uid)
                        rtt_position = await self._async_fetch_rtt_position(rtt_uid, update_start_time)

                    # RTT-first: if RTT has position, keep en-route tracking alive even if destination board lags.
                    if rtt_position:
                        self._enroute_failure_count = 0
                        arrival_data = await self._async_fetch_arrival_data()
                        new_attrs = self._attr_extra_state_attributes.copy()
                        new_attrs["last_update_time"] = update_start_time.isoformat(timespec='milliseconds')
                        new_attrs["last_update_trigger"] = (
                            "RTT+Arrival Poll" if arrival_data and arrival_data.get("success") else "RTT Poll"
                        )

                        if new_attrs.get("tracked_train_details") is None:
                            new_attrs["tracked_train_details"] = {}
                        new_attrs["tracked_train_details"]["rtt_uid"] = rtt_uid
                        ...
                        delay_reason = None
                        cancel_reason = None
                        arrival_time_str = "Unknown"

                        if arrival_data and arrival_data.get("success"):
                            arrival = arrival_data.get("arrival", {})
                            expected_arrival = arrival.get("expected_arrival")
                            platform = arrival.get("platform")
                            is_cancelled = arrival.get("is_cancelled", False)
                            delay_reason = arrival.get("delay_reason")
                            cancel_reason = arrival.get("cancel_reason")

                            if platform:
                                new_attrs["tracked_train_details"]["platform"] = platform
                            if isinstance(expected_arrival, datetime):
                                arrival_time_str = expected_arrival.strftime("%H:%M")
                            elif expected_arrival:
                                arrival_time_str = str(expected_arrival)
                            new_attrs["tracked_train_details"]["expected_arrival_time"] = arrival_time_str

                        # Cancellation and arrival are still best confirmed by destination board if available.
                        if is_cancelled:
                            _LOGGER.info(
                                "%s: Train cancelled at destination %s. Stopping tracking.",
                                entity_id_log,
                                self._destination_crs,
                            )
                            new_value = "Cancelled"
                            new_attrs["status"] = "Cancelled"
                            new_attrs["info"] = f"Train cancelled. {cancel_reason or ''}"
                            new_attrs["tracking_mode"] = "Completed"
                            self._enroute_tracking_enabled = False
                        ...
                        has_arrived = False
                        if isinstance(expected_arrival, datetime):
                            has_arrived = expected_arrival < update_start_time
                        elif isinstance(expected_arrival, str) and expected_arrival.lower() == "on time":
                            sched_arrival = arrival.get("scheduled_arrival")
                            has_arrived = isinstance(sched_arrival, datetime) and sched_arrival < update_start_time

                        # RTT-only arrival fallback when destination board has not yet updated.
                        dest_crs = (self._destination_crs or "").upper()
                        if (
                            not has_arrived
                            and dest_crs
                            and (rtt_position.get("last_station_crs") or "").upper() == dest_crs
                            and not rtt_position.get("next_station")
                        ):
                            has_arrived = True

                        if has_arrived:
                            dest_name = STATION_MAP.get(self._destination_crs, self._destination_crs)
                            _LOGGER.info("%s: RTT/arrival indicates train arrived at %s", entity_id_log, dest_name)
                            new_value = f"Arrived at {dest_name}"
                            new_attrs["status"] = "Arrived"
                            new_attrs["info"] = f"Train arrived at {dest_name}"
                            new_attrs["tracking_mode"] = "Completed"
                            self._enroute_tracking_enabled = False
                            self._tracking_stopped = True
                            self._tracking_stopped_at = update_start_time
                        ...
                        new_value = "En Route"
                        new_attrs["status"] = "En Route"
                        new_attrs["tracking_mode"] = "RTT-Polling"
                        dest_name = STATION_MAP.get(self._destination_crs, self._destination_crs)

                        info_parts = []
                        last_station = rtt_position.get("last_station")
                        next_station = rtt_position.get("next_station")
                        if last_station and next_station:
                            info_parts.append(f"Between {last_station} and {next_station}")
                        elif next_station:
                            info_parts.append(f"Next stop: {next_station}")
                        elif last_station:
                            info_parts.append(f"Last reported: {last_station}")

                        info_parts.append(f"Arriving at {dest_name}")

                        if arrival_time_str != "Unknown":
                            info_parts.append(f"ETA: {arrival_time_str}")
                        elif rtt_position.get("next_station_time"):
                            info_parts.append(f"Next timing point: {rtt_position['next_station_time']}")

                        if platform:
                            info_parts.append(f"Platform {platform}")
                        if delay_reason:
                            info_parts.append(f"Delayed: {delay_reason}")
                        elif rtt_position.get("is_delayed"):
                            info_parts.append("Delayed")
                    ...
                    _LOGGER.info(
                        "%s: RTT unavailable, polling arrival board at %s for train from %s",
                        entity_id_log,
                        self._destination_crs,
                        self._origin_crs
                    )
                    enroute_data = await self._async_fetch_arrival_data()
                    self._last_enroute_check = update_start_time

                    if enroute_data and enroute_data.get("success"):
                        self._enroute_failure_count = 0
                        # Process arrival data
                        new_attrs = self._attr_extra_state_attributes.copy()
                        new_attrs["last_update_time"] = update_start_time.isoformat(timespec='milliseconds')
                        new_attrs["last_update_trigger"] = "Arrival Poll"

                        # Extract arrival information
                        arrival = enroute_data.get("arrival", {})
                        expected_arrival = arrival.get("expected_arrival")
                        platform = arrival.get("platform")
                        is_cancelled = arrival.get("is_cancelled", False)
                        delay_reason = arrival.get("delay_reason")
                        ...
                        # Update tracked_train_details with arrival info
                        if new_attrs.get("tracked_train_details") is None:
                            new_attrs["tracked_train_details"] = {}

                        # Update platform if we have it
                        if platform:
                            new_attrs["tracked_train_details"]["platform"] = platform
                        ...
                        if isinstance(expected_arrival, datetime):
                            ...
                            arrival_time_str = str(expected_arrival) if expected_arrival else "Unknown"

                        new_attrs["tracked_train_details"]["expected_arrival_time"] = arrival_time_str

                        # Check if train has been cancelled
                        if is_cancelled:
                            _LOGGER.info(
                                "%s: Train cancelled at destination %s. Stopping tracking.",
                                entity_id_log,
                            ...
                            self._tracking_stopped = True
                            self._tracking_stopped_at = update_start_time
                            # Clear high-frequency polling request
                            self._coordinator.request_high_frequency_polling(enable=False)

                            self._update_state_and_attrs(new_value, new_attrs, force_write=True)
                            return

                        # Check if train has arrived (ETA is in the past or "On time" with scheduled time past)
                        has_arrived = False
                        if isinstance(expected_arrival, datetime):
                            if expected_arrival < update_start_time:
                                has_arrived = True
                        elif isinstance(expected_arrival, str) and expected_arrival.lower() == "on time":
                            sched_arrival = arrival.get("scheduled_arrival")
                            if isinstance(sched_arrival, datetime) and sched_arrival < update_start_time:
                                has_arrived = True

                        if has_arrived:
                            _LOGGER.info(
                                "%s: Train arrived at destination %s. Stopping tracking.",
                                entity_id_log,
                                self._destination_crs
                            )
                            dest_name = STATION_MAP.get(self._destination_crs, self._destination_crs)
                        ...
                        # Still en-route with arrival-board-only data
                        new_value = "En Route"
                        new_attrs["status"] = "En Route"
                        new_attrs["tracking_mode"] = "Arrival-Polling"
                        dest_name = STATION_MAP.get(self._destination_crs, self._destination_crs)
                        info_parts = [f"Arriving at {dest_name}"]
                        if arrival_time_str != "Unknown":
                            info_parts.append(f"ETA: {arrival_time_str}")
                        if platform:
                            info_parts.append(f"Platform {platform}")
                    ...
                    # Arrival boards are often eventually consistent right after departure.
                    # Allow a few misses before giving up.
                    if self._enroute_failure_count < self._max_enroute_failures:
                        _LOGGER.info(
                            "%s: Arrival board poll miss %d/%d for %s. Will retry.",
                            entity_id_log,
                            self._enroute_failure_count,
                            self._max_enroute_failures,
                            self._destination_crs,
                        )
                        return

                    # Arrival poll failed repeatedly - disable arrival polling
                    _LOGGER.warning(
                        "%s: Arrival board polling failed %d times for train to %s. Disabling arrival polling.",
                        entity_id_log,
                        self._enroute_failure_count,
                        self._destination_crs
                    )
                    ...
                    # Get departure time from previous state
                    prev_details = self._attr_extra_state_attributes.get("tracked_train_details", {})
                    last_expected_time_str = prev_details.get("expected_departure_time", "?")

                    new_attrs = self._attr_extra_state_attributes.copy()
                    new_value = f"Departed ~{last_expected_time_str}"
                    new_attrs["status"] = "Departed"
                    new_attrs["info"] = f"Train departed ~{last_expected_time_str}. En-route tracking not available for this service."
                    new_attrs["tracking_mode"] = "Completed"
                    new_attrs["last_update_time"] = update_start_time.isoformat(timespec='milliseconds')
                    ...
                    _LOGGER.debug("%s: En-route check not needed yet (last check: %s ago)",
                                entity_id_log,
                                (update_start_time - self._last_enroute_check).total_seconds() if self._last_enroute_check else "never")
                    return

            # --- Store previous state BEFORE this update runs ---
            previous_native_value = self._attr_native_value
            ...
            if not self.coordinator:
                ...
                if coord_data.get("api_error"):
                    ...
                    if trains_candidate is None:
                        ...
                        # Debug: Log what trains are in all_trains
                        train_ids_in_feed = [train.get("serviceId", f"{train.get('scheduled', '?')}_{train.get('terminus', '?')}") for train in all_trains]
                        _LOGGER.debug("%s: all_trains contains %d trains: %s",
                                      entity_id_log, len(all_trains), train_ids_in_feed)
            ...
            if process_trains:
                ...
                if not self._initialized:
                    ...
                    if initial_best_train and initial_arrival_dt:
                        ...
                        # Check if we have a real serviceId from API
                        real_service_id = initial_best_train.get("serviceId")
                        if real_service_id:
                            self._tracked_train_id = real_service_id
                            self._has_real_service_id = True
                        else:
                            self._tracked_train_id = f"{sched_str}_{terminus}"
                            self._has_real_service_id = False
                            _LOGGER.debug("%s: No API serviceId, using constructed ID: %s",
                                         entity_id_log, self._tracked_train_id)

                        # Store destination CRS for arrival board polling
                        # Extract from the destinations list matching our target
                        self._destination_crs = None
                        destinations_list = initial_best_train.get("destinations", [])
                        for dest in destinations_list:
                            dest_crs = dest.get("crs", "")
                            # Match against target_destination (e.g., "YRK", "LDS")
                            if dest_crs.upper() == self._target_destination.upper():
                                self._destination_crs = dest_crs
                                break

                        # Fallback: try matching by name
                        if not self._destination_crs:
                            target_name = STATION_MAP.get(self._target_destination, self._target_destination).lower()
                            for dest in destinations_list:
                                dest_name = dest.get("name", "").lower()
                                if dest_name == target_name:
                                    self._destination_crs = dest.get("crs")
                                    break

                        _LOGGER.debug(
                            "%s: Stored destination CRS: %s (target: %s, terminus: %s)",
                            entity_id_log,
                            self._destination_crs,
                            self._target_destination,
                            terminus
                        )
                        ...
                        if self._tracked_scheduled_departure_dt:
                            try:
                                ...
                                # First, try to get terminus CRS from destinations list (should be the last one or match terminus name)
                                destinations_list = initial_best_train.get("destinations", [])
                                if destinations_list:
                                    # Check if any destination matches the terminus name
                                    terminus_name_upper = terminus.upper()
                                    for dest in destinations_list:
                                        dest_name = dest.get("name", "").upper()
                                        if terminus_name_upper in dest_name or dest_name in terminus_name_upper:
                                            terminus_crs = dest.get("crs")
                                            break

                                # Fallback: try reverse lookup from STATION_MAP
                                if not terminus_crs:
                                    reverse_map = {name.upper(): code for code, name in STATION_MAP.items()}
                                    terminus_crs = reverse_map.get(terminus.upper())

                                # Final fallback: if still unknown, use the last destination.
                                # Note this may be filtered to the user's target destination.
                                if not terminus_crs and destinations_list:
                                    terminus_crs = destinations_list[-1].get("crs")

                                # Use terminus for RTT matching, or fall back to user's destination if terminus unknown
                                rtt_destination_crs = terminus_crs or self._destination_crs

                                _LOGGER.info("%s: Fetching RTT UID for %s->%s at %s (operator: %s, terminus: %s)",
                                            entity_id_log, self._origin_crs, rtt_destination_crs,
                ...
                # ... (Phase 2: Tracking & Re-evaluation remains unchanged, including finding tracked train, triggers, comparison, switching, and vanishing logic) ...
                    ...
                    if not self._tracked_train_id:
                        ...
                        _LOGGER.debug("%s: Searching for tracked train %s in %d trains. Found: %s",
                                      entity_id_log, self._tracked_train_id, len(all_trains),
                                      "YES" if found_tracked_train else "NO")
                        ...
                        if found_tracked_train:
                            ...
                            if trigger_reevaluation:
                                ...
                                if best_alternative_train and alt_arrival_dt:
                                    ...
                                    if current_tracked_arrival and alt_arrival_dt < (current_tracked_arrival - REEVALUATION_BETTER_THRESHOLD):
                                        ...
                                        # Check if new train has real serviceId
                                        new_real_service_id = best_alternative_train.get("serviceId")
                                        if new_real_service_id:
                                            self._tracked_train_id = new_real_service_id
                                            self._has_real_service_id = True
                                        else:
                                            self._tracked_train_id = f"{new_sched_str}_{new_terminus}"
                                            self._has_real_service_id = False

                                        # Store destination CRS for arrival board polling
                                        # Extract from the destinations list matching our target
                                        self._destination_crs = None
                                        destinations_list = best_alternative_train.get("destinations", [])
                                        for dest in destinations_list:
                                            dest_crs = dest.get("crs", "")
                                            # Match against target_destination (e.g., "YRK", "LDS")
                                            if dest_crs.upper() == self._target_destination.upper():
                                        ...
                                        _LOGGER.debug(
                                            "%s: Stored destination CRS for switched train: %s (target: %s, terminus: %s)",
                                            entity_id_log,
                                            self._destination_crs,
                                            self._target_destination,
                                            new_terminus
                            ...
                            _LOGGER.info("%s: Tracked train ID %s not found in current data feed.",
                                         entity_id_log, self._tracked_train_id)

                            # Determine state based on *previous* known state
                            ...
                            # Check if we should switch to arrival board polling
                            # We need origin and destination station codes for this
                            can_track_arrival = (
                                self._destination_crs and  # Have destination station code
                                self._origin_crs and  # Have origin station code
                                self._tracked_train_id and  # Have tracked train ID
                                last_status != "Cancelled" and  # Train wasn't cancelled
                                last_expected_dt and  # Know when it departed
                                update_start_time > (last_expected_dt + timedelta(minutes=2))  # Likely departed
                            )

                            if can_track_arrival and not self._enroute_tracking_enabled:
                                # Switch to arrival board polling mode
                                _LOGGER.info(
                                    "%s: Switching to arrival board polling for train to %s (departed ~%s)",
                                    entity_id_log,
                                    self._destination_crs,
                                    last_expected_time_str
                                )
                                self._enroute_tracking_enabled = True  # Reuse this flag for arrival polling
                                self._last_enroute_check = None  # Force immediate check
                                self._enroute_failure_count = 0

                                new_native_value = f"En Route"
                                new_attrs["status"] = "En Route"
                                new_attrs["info"] = (
                                    f"Train departed ~{last_expected_time_str}. "
                                    f"Polling arrival board at {self._destination_crs}..."
                                )
                                new_attrs["tracking_mode"] = "Arrival-Polling"
                                new_attrs["re_evaluation_status"] = "N/A (En-Route)"
                                if last_status != "En Route":
                                    event_to_fire = {
                                        'name': f"{DOMAIN}_status_changed",
                                        'data': {"status": "En Route"},
                                    }

                                # Keep tracked_train_details from previous state
                                # Will be updated by arrival polling

                                # Don't set current_train_data - we'll fetch arrival data instead
                                current_train_data = None

                            else:
                                # If the service briefly disappears around departure time, keep tracking active
                                # and wait for the next poll before declaring loss.
                                if last_expected_dt and update_start_time <= (last_expected_dt + timedelta(minutes=2)):
                                    _LOGGER.info(
                                        "%s: Train %s temporarily missing near departure (%s). Waiting for next poll.",
                                        entity_id_log,
                                        self._tracked_train_id,
                                        last_expected_time_str,
                                    )
                                    new_native_value = f"Departing ~{last_expected_time_str}"
                                    new_attrs["status"] = "Departing"
                                    new_attrs["info"] = (
                                        f"Train temporarily missing from departure board near {last_expected_time_str}. "
                                        "Waiting for departure confirmation..."
                                    )
                                    if last_status != "Departing":
                                        event_to_fire = {
                                            'name': f"{DOMAIN}_status_changed",
                                    ...
                                    # Cannot track via arrival board - stop tracking
                                    if not self._destination_crs:
                                        reason = "No destination station code"
                                    elif not self._origin_crs:
                                        reason = "No origin station code"
                                    elif last_status == "Cancelled":
                                        reason = "Train was cancelled"
                                    elif not last_expected_dt:
                                        reason = "Unknown departure time"
                                    else:
                                        reason = "Unknown reason"

                                    _LOGGER.info(
                                        "%s: Cannot enable arrival polling for train %s. Reason: %s",
                                        entity_id_log,
                                        self._tracked_train_id,
                                        reason
                                    )
            ...
            if current_train_data:
                ...
                # Add RealTimeTrains UID if available
                details["rtt_uid"] = self._tracked_uid
                ...
                # --- Handle Train Switch Baseline Reset ---
                # If we just switched trains, establish a clean baseline for change detection
                # by updating previous_attributes to reflect the NEW train's current state.
                # This prevents comparing the new train against the old train in future cycles.
                if change_flags.get("switched_train"):
                    _LOGGER.debug("%s: Train switched - establishing clean baseline for new train %s",
                                 entity_id_log, self._tracked_train_id)
                    # Create a baseline previous_attributes with the new train's initial state
                    baseline_attrs = previous_attributes.copy()
                    baseline_attrs["tracked_train_details"] = details.copy()
                    baseline_attrs["status"] = details.get("status", "Unknown")
                    # Update stored previous_attributes so next cycle compares new train against itself
                    self._previous_attributes = baseline_attrs
                    _LOGGER.debug("%s: Baseline established. Next cycle will track changes for new train.", entity_id_log)
            ...
            # Note: _update_state_and_attrs has its own guard to skip writes if nothing changed
            _LOGGER.debug(
                "%s UPDATE: PRE-WRITE STATE. Native Value: '%s' (Type: %s). Attributes: %s",
                entity_id_log,
                new_native_value,
                type(new_native_value).__name__,
                new_attrs,
            )
            ...
            if fire_this_event and (force_write or new_attrs != previous_attributes or new_native_value != previous_native_value):
                 ...
                 if isinstance(tracked_details, dict):
                      base_event_data.update({
                         ...
                         "rtt_uid": tracked_details.get("rtt_uid"),  # RealTimeTrains UID
                         # <<< END CHANGE >>>
                      })
            ...
            # --- Request Fast Polling if Tracked Train is Approaching or Recently Departed ---
            if self._initialized and self._tracked_scheduled_departure_dt and not self._tracking_stopped:
                # Use expected departure time (accounts for delays) for imminent check
                # Fall back to scheduled time if expected time not available
                departure_time_for_check = self._tracked_scheduled_departure_dt
                if current_train_data and current_train_data.get("expected_departure_iso"):
                    try:
                        departure_time_for_check = self._parse_train_time(current_train_data["expected_departure_iso"])
                    except:
                        pass  # Fall back to scheduled time

                time_until_departure = (departure_time_for_check - update_start_time).total_seconds() / 60

                # Enable fast polling if:
                # 1. Train departs within 10 minutes (approaching), OR
                # 2. Train departed up to 5 minutes ago (to detect disappearance from departure board)
                if -5 < time_until_departure <= HIGH_FREQUENCY_REFRESH:
                    _LOGGER.debug(
                        "%s: Tracked train departs in %.1f minutes - requesting high-frequency polling",
                        entity_id_log,
                        time_until_departure
                    )
                    # Request coordinator to enable high-frequency polling
                    self._coordinator.request_high_frequency_polling(enable=True)
                else:
                    # Train not imminent and not recently departed, clear the request
                    self._coordinator.request_high_frequency_polling(enable=False)
